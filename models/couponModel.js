...
exports.createCoupon = async (couponData) => {
  ...
  const query = `
    INSERT INTO coupons 
    (code, discountType, discountValue, minOrderAmount, usageLimit, validFrom, validTill)
    VALUES (?, ?, ?, ?, ?, ?, ?)`;
...
// EDIT COUPON
exports.editCoupon = async (couponId, couponData) => {
  const {
    code,
    discountType,
    discountValue,
...
exports.deleteCoupon = async (couponId) => {
  const [result] = await db.query("DELETE FROM coupons WHERE id = ?", [
    couponId,
  ]);
  return result;
};
...
exports.getUserCoupons = async (userId) => {
  const query = `
    SELECT uc.isUsed, uc.expiresAt, c.code, c.discountType, 
           c.discountValue, c.minOrderAmount, c.validTill
    FROM user_coupons uc
    JOIN coupons c ON uc.couponId = c.id
    WHERE uc.userId = ? 
      AND (uc.expiresAt IS NULL OR uc.expiresAt > NOW())`;
...
// VALIDATE USER COUPON
exports.validateUserCoupon = async (userId, couponCode, cartTotal) => {
  const query = `
    SELECT uc.*, c.discountType, c.discountValue, c.minOrderAmount
    FROM user_coupons uc
    JOIN coupons c ON uc.couponId = c.id
  ...
  const [results] = await db.query(query, [userId, couponCode]);
...
// REWARD COUPON TO USER
exports.rewardCouponToUser = async (userId, couponCode) => {
  // Step 1: Find coupon
  const couponQuery = `
    SELECT * FROM coupons 
    WHERE code = ? AND validFrom <= NOW() 
      AND (validTill IS NULL OR validTill >= NOW())
    LIMIT 1`;

  const [coupons] = await db.query(couponQuery, [couponCode]);
  ...
  // Step 2: Check if user already has this coupon
  const userCouponCheckQuery = `
    SELECT * FROM user_coupons WHERE userId = ? AND couponId = ?`;

  const [userCoupons] = await db.query(userCouponCheckQuery, [
    userId,
    coupon.id,
  ]);

  if (userCoupons.length > 0) {
    throw new Error("User already earned this coupon");
  }

  // Step 3: Reward it
  const expiresAt =
    coupon.validTill || new Date(Date.now() + 7 * 24 * 60 * 60 * 1000); // 7 days default

  const insertQuery = `
    INSERT INTO user_coupons (userId, couponId, expiresAt) 
    VALUES (?, ?, ?)`;

  const [result] = await db.query(insertQuery, [userId, coupon.id, expiresAt]);

  return { message: "Coupon rewarded successfully", coupon };
};
